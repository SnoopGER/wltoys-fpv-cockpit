...
$Interface = "WiFi 3"
$Car2BindIp = "172.16.11.2"
$Root = Split-Path -Parent $MyInvocation.MyCommand.Path
$EnvFile = Join-Path $Root ".env.local"

Write-Host "Preparing $Interface for car2 with static IP $Car2BindIp/24"
...
Get-NetIPAddress -InterfaceAlias $Interface -AddressFamily IPv4 -ErrorAction SilentlyContinue |
    Where-Object { $_.IPAddress -ne $Car2BindIp } |
    ForEach-Object {
        try {
...
if (-not (Get-NetIPAddress -InterfaceAlias $Interface -IPAddress $Car2BindIp -ErrorAction SilentlyContinue)) {
    try {
        New-NetIPAddress -InterfaceAlias $Interface -IPAddress $Car2BindIp -PrefixLength 24 -ErrorAction Stop | Out-Null
        Write-Host "$Interface static IP set to $Car2BindIp/24"
    } catch {
        Write-Host "Failed to set static IP on ${Interface}: $($_.Exception.Message)"
    }
} else {
    Write-Host "$Interface already has $Car2BindIp"
}
...
if ($content -match "^FPV_CAR2_BIND_IP=") {
    $content = $content -replace "^FPV_CAR2_BIND_IP=.*$", "FPV_CAR2_BIND_IP=$Car2BindIp"
} else {
    $content += "FPV_CAR2_BIND_IP=$Car2BindIp"
}
