...
// ── Guest Code Functions ──────────────────────────────────
async function redeemCode() {
  const input = $('guestCodeInput');
  if (!input) return;
  const code = input.value.trim();
  if (!code) {
    addLog('ERR', 'Please enter a drive code.');
    return;
  }
  try {
...
function initGuestTimer() {
  const timerEl = $('guestTimeLeft');
  if (!timerEl || !document.body.dataset.userId?.startsWith('guest-')) return;
  
  async function updateGuestTimer() {
    try {
      const resp = await fetch('/api/guest/remaining', { credentials: 'include' });
      const data = await resp.json();
      if (data.ok) {
        ...
        if (data.remaining <= 0) {
          ...
          addLog('SYS', '⏱️ Drive time expired!');
          stopMotor();
        }
      }
    } catch (e) {}
  ...
  updateGuestTimer();
  guestTimerInterval = setInterval(updateGuestTimer, 1000);
}

// Admin: Generate guest code
async function generateGuestCode() {
  const durationEl = $('guestDuration');
  const duration = durationEl ? parseInt(durationEl.value) : 10;
  try {
    const resp = await fetch('/api/guest/generate', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      credentials: 'include',
    ...
    if (data.ok) {
      ...
      addLog('SYS', '🎟️ Code generated: ' + data.code);
      refreshGuestCodes();
    } else {
      addLog('ERR', 'Code generation failed: ' + (data.error || 'unknown'));
    }
  } catch (e) {
    addLog('ERR', 'Network error: ' + e.message);
  }
...
function copyCode() {
  ...
  if (codeEl) {
    navigator.clipboard.writeText(codeEl.textContent).then(() => {
      addLog('SYS', '📋 Code copied to clipboard');
    }).catch(() => {
...
// Admin: Refresh guest code list
async function refreshGuestCodes() {
  if (!isAdmin()) return;
  try {
    const resp = await fetch('/api/guest/codes', { credentials: 'include' });
    const data = await resp.json();
    if (data.ok) {
      const list = $('guestCodeList');
      if (!list) return;
      if (data.codes.length === 0) {
        list.innerHTML = '<div class="dim-item">No codes generated yet</div>';
        return;
      }
      list.innerHTML = data.codes.map(c => {
        ...
        return `<div class="guest-code-row">
          ...
          ${c.active ? `<button class="btn btn-red btn-small" onclick="revokeCode('${c.code}')">REVOKE</button>` : ''}
        </div>`;
      }).join('');
    }
  } catch (e) {}
...
async function revokeCode(code) {
  try {
    const resp = await fetch('/api/guest/revoke', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      credentials: 'include',
      body: JSON.stringify({ code }),
